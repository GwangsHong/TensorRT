...
#print(qkv2_plg_creator)
#print(skln_plg_creator)
#print(gelu_plg_creator)
#print(emln_plg_creator)
...
def attention_layer_opt(prefix, config, init_dict, network, input_tensor, imask):
    ...
    pf_type_id = trt.PluginField("type_id", np.array([0], dtype=np.float32), trt.PluginFieldType.INT32)
    ...
    pfc = trt.PluginFieldCollection([pf_type_id, pf_hidden_size, pf_num_heads, pf_has_mask])
...
def transformer_layer_opt(prefix, config, init_dict, network, input_tensor, imask):
    ...
    gelu_bias_shape = mid_dense_out.shape[3]
    

    pf_type_id = trt.PluginField("type_id", np.array([0], dtype=np.float32), trt.PluginFieldType.INT32)

    pfc = trt.PluginFieldCollection([pf_type_id])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='TensorRT 7.0 BERT Sample', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
